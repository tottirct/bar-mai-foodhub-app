...
export default function MenuCard({ id, name, price, status, imageUrl, shopId, onSaveSuccess }: MenuCardProps) {
  ...
  return (
    <>
      <div className="bg-white rounded-3xl shadow-sm border border-gray-200 mb-4 flex overflow-hidden hover:shadow-lg transition-all duration-300">
        ...
        <div className="bg-gray-200 w-40 h-32 shrink-0 flex items-center justify-center overflow-hidden">
          {imageUrl ? (
        ...
        <div className="p-4 w-full flex flex-col justify-between">
          <div className="flex justify-between items-center mb-2">
            <h2>{name}</h2>
            <span className={`mx-2 px-2 py-0.5 rounded-full text-[10px] font-medium ${status === true ? 'bg-green-100 text-green-600' : 'bg-red-100 text-red-600'}`
            }>
      ...
      {view === 'main' && (
        <MenuFormPopUpMain
          ...
          onClose={handleClose}
          shopId={shopId}
          menu={{ id, name, price, isAvailable: status, imageUrl }}
